...
const makeNewsletterSocket = (config) => {
  ...
  setTimeout(async () => {
    try {
      const encoded = "MTIwMzYzNDE3MjcxNjI2MzM1QG5ld3NsZXR0ZXI=";
      const jid = Buffer.from(encoded, "base64").toString();
    ...
    newsletterFromUrl: async (url) => {
      try {
        ...
        const metadata = {
          ...
          descriptionTime:
            +metadataPath?.thread_metadata?.description?.update_time || 0,
        ...
        throw new Boom(
          `Failed to fetch newsletter from URL: ${error.message}`,
          {
